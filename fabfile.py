...
INSERT INTO wealth1 SELECT CONCAT( 3,0,(@rownum := @rownum + 1)) as id, t1.player_id, t1.wealth_id, t1.gold, t1.time_id, t1.create_time FROM wealth  t1, (select @rownum := 0) r
...
def Excute_sql(sql_command, db1, db2, id1, id2):
    ...
    try:
        ...
        con = mdb.connect(host='localhost', user='root', passwd='123456', db=db1, charset='utf8')
        ...
        for row in results:
            ...
            if str(row[0]) in ["money_tree_reward"]:
                ...
                #清空主表和副表  ----清空主表，然后不合并
            if "rank" in str(row[0]) or "tong_battle" in str(row[0]) or str(row[0]) in ["sell_item_price", "arena", "player_rename"]:
...
if __name__ == "__main__":
    sql = "show tables;"
    masterdb = 'hefu1'  # 主数据库名称              #即将jtyl2合并到jtyl1
    masterid = 1
    tempdb = 'hefu3'  # 被合并的数据库名称
    tempid = 3
