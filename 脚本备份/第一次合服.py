...
INSERT INTO wealth1 SELECT CONCAT( 27,0,(@rownum := @rownum + 1)) as id, t1.player_id, t1.wealth_id, t1.gold, t1.time_id, t1.create_time FROM wealth  t1, (select @rownum := 0) r;
...
def Excute_sql(sql_command, db1, db2, id1, id2):
    ...
    try:
        ...
        con = mdb.connect(host='localhost', user='root', passwd='Jtyl2017!@#$', db=db1, charset='utf8')
        ...
        for row in results:
            ...
            if str(row[0]) in ["sequence","server_config","in_test_reward","in_test_reward2"]:
                continue
            ...
            # 副表id值加10000000，然后合并           如果为第一次和服就是10000000
            if str(row[0]) in ["auction_item","master_relation", "auction_item_log", "yunying_time"]:
                cur.execute("UPDATE " + db2 + "." + row[0] + " SET id=id+10000000")
                con.commit()

            # 副表rid值加10000000，然后合并
            if str(row[0]) in ["master_daily_target", "master_graduate_target"]:
                cur.execute("UPDATE " + db2 + "." + row[0] + " SET rid=rid+10000000")
                con.commit()
...
if __name__ == "__main__":
    sql = "show tables;"
    masterdb = 'jtyl2022'  # 主数据库名称              #即将jtyl2合并到jtyl1
    masterid = 2022
    tempdb = 'jtyl2023'  # 被合并的数据库名称
    tempid = 2023
